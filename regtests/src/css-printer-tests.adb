-----------------------------------------------------------------------
--  css-printer-tests -- Unit tests for CSS printer
...
package body CSS.Printer.Tests is
   ...
   begin
      return Util.Tests.Format ("Test CSS printer " & To_String (T.Name));
   end Name;
   ...
   begin
      Doc.Set_Href (Name);
      Output.Compress := T.Compress;
      if not T.Compress then
         Output.Indent_Level := 4;
         Output.Full_Semi := True;
      end if;
      Ada.Text_IO.Create (Output.File, Ada.Text_IO.Out_File, To_String (T.Result));
      declare
      ...
      Util.Tests.Assert_Equal_Files (T, Ada.Strings.Unbounded.To_String (T.Expect),
                                     To_String (T.Result), "Print of " & Name & " failed");

   exception
      when others =>
         if Ada.Text_IO.Is_Open (Output.File) then
            Ada.Text_IO.Close (Output.File);
         end if;
         raise;
   ...
   procedure Add_Tests (Suite : in Util.Tests.Access_Test_Suite) is
      ...
      function Create_Test (Name    : in String;
                            Path    : in String;
                            Prefix  : in String) return Test_Case_Access is
      ...
      begin
         ...
         while More_Entries (Search) loop
            ...
            begin
               ...
               then
                  if Compress then
                     Tst := Create_Test (Name, Path & "/" & Simple, "compress/");
                  else
                     Tst := Create_Test (Name, Path & "/" & Simple, "printer/");
                  end if;
                  if Tst /= null then
                     Tst.Compress := Compress;
   ...
   begin
      Add_Parser_Tests ("regtests/files/printer", False);
      Add_Parser_Tests ("regtests/files/compress", True);
   end Add_Tests;
